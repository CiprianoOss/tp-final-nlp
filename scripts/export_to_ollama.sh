...
python "$LLAMA_CPP_DIR/convert_hf_to_gguf.py" \
  "$MERGED_DIR" \
  --outfile "$GGUF_DIR/fireball-qwen3-4b-f16.gguf" \
  --outtype f16
...
"$LLAMA_CPP_DIR/build/bin/llama-quantize" \
  "$GGUF_DIR/fireball-qwen3-4b-f16.gguf" \
  "$GGUF_DIR/fireball-qwen3-4b-q4_k_m.gguf" \
  Q4_K_M

OLLAMA_GGUF="$(cd "$GGUF_DIR" && pwd)/fireball-qwen3-4b-q4_k_m.gguf"
